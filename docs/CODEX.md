...
- `backend/app/services/manual_render_service.py` erzeugt pro Sprache `workspace/output/{video_id}/manual_{lang}.docx` im A5-Querformat mit Calibri 10 pt und 1 cm Raendern. Bilder bleiben eingebettete Bilder, Texte bleiben bearbeitbarer Word-Text. Szenen werden als offizielle Handbuchabschnitte mit Bild-/Texttabellen formatiert; `body` und `speaker_notes` werden pro Szene verlustfrei auf die Bilder verteilt statt als separater Gesamtblock ausgegeben. Jedes Bild einer Szene belegt eine eigene Seite: Die Tabelle hat 2 Zeilen (1 Spalte), oben das Bild (volle Seitenbreite, max. 9 cm Hoehe, Seitenverhaeltnis wird beibehalten), unten der zugehoerige Text. Zwischen Bildern einer Szene wird ein Seitenumbruch eingefuegt.
